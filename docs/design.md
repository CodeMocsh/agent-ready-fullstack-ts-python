...
| design-system conformance | `devtools/conformance.mjs` (6 checks) | none — no analogue exists |
...
from its own diff to `AGENTS.md`. Its six checks are `raw-colour`, `palette-utility`,
`named-colour`, `arbitrary-spacing`, `arbitrary-type`, `raw-type-declaration`, configured by
a `conformance` block (`themeFiles`, `allow`, `exclude`) and chained into `lint`/`lint:check`.
