...
The last two tests are the tenancy contract. Postgres keeps it with a forced policy and memory
keeps it with a dictionary, and the suite does not care which -- only that neither can be
talked into answering for a tenant it was not scoped to.
"""
...
async def test_two_tenants_do_not_see_each_others_tasks(database: Database) -> None:
...
async def test_a_store_cannot_be_had_without_a_tenant(database: Database) -> None:
    """On both substrates, and at construction. There is no unscoped store to hold, which is
    what stops a route from forgetting -- and an unset tenant answers emptily under the
