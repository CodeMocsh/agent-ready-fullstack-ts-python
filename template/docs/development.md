...
The two halves check their own end: the backend regenerates the spec from the code and
diffs it, the frontend regenerates the types from the spec and diffs those. Neither needs
the other's toolchain, which is what lets one half be verified alone.
...
Both runs are in `make pre-commit`. The mock-mode pass rides along in `pnpm test`; the
live pass is `make test-contract`, and it is the one check that needs both toolchains at
once.

It is also the one place the single-toolchain rule is broken on purpose. Every other check
here passes on a project whose frontend cannot reach its backend at all, which is why this
one is worth the setup it costs.
...
the git hook runs, and — since this project ships no workflow — the whole of what enforces
anything. `make hooks` arms it; `make install` does that for you.
...
None of these is in the gate: the two Playwright targets need a browser binary, the live
one needs a second terminal, and `db-test` needs a Docker daemon. A gate that fetches any of
those is a gate people learn to commit around, and `tests/test_gate.py` asserts all three
stay out of it.
...
- **backend** — `exclude-newer = "14 days"` in `backend/pyproject.toml`, repeated in the
  root `Makefile` because uvx and tool invocations run outside project discovery.
...
FastAPI and pydantic both write part of `openapi.json`, which is committed and diffed by
`make openapi-check`, so an unbounded floor means a release published next month can fail a
project that nobody touched. Raising a cap is a deliberate edit followed by `make openapi`.

Both lockfiles are committed, so every clone installs the tree the last one resolved.
